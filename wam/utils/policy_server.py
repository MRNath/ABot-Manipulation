...
from wam_client.msgpack_numpy import Packer, unpackb
...
class WebsocketPolicyServer:
    ...
    async def run(self):
        process_request = _health_check if _NEW_WEBSOCKETS_API else _legacy_health_check
    ...
    async def _handler(self, websocket):
        ...
        while True:
            try:
                ...
                logger.info(
                    "Request from %s keys=%s",
                    websocket.remote_address,
                    sorted(obs.keys()) if isinstance(obs, dict) else type(obs).__name__,
                )
                ...
                logger.info(
                    "Request from %s finished in %.3fs",
                    websocket.remote_address,
                    infer_time,
                )
...
def _legacy_health_check(path, request_headers):
    if path == "/healthz":
